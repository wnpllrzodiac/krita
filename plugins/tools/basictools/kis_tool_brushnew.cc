...
void KisToolBrushNew::addSmoothingAction(int enumId, const QString &id)
{
    /**
     * KisToolBrushNew is the base of several tools, but the actions
...
KisToolBrushNew::~KisToolBrushNew()
{
}

void KisToolBrushNew::activate(const QSet<KoShape*> &shapes)
{
...
void KisToolBrushNew::deactivate()
{
...
int KisToolBrushNew::smoothingType() const
{
    return smoothingOptions()->smoothingType();
}

bool KisToolBrushNew::smoothPressure() const
{
    return smoothingOptions()->smoothPressure();
}

int KisToolBrushNew::smoothnessQuality() const
{
    return smoothingOptions()->smoothnessDistance();
}

qreal KisToolBrushNew::smoothnessFactor() const
{
    return smoothingOptions()->tailAggressiveness();
}

void KisToolBrushNew::slotSetSmoothingType(int index)
{
    /**
...
void KisToolBrushNew::updateSmoothnessDistanceLabel()
{
...
void KisToolBrushNew::slotSetSmoothnessDistance(qreal distance)
{
    smoothingOptions()->setSmoothnessDistance(distance);
    Q_EMIT smoothnessQualityChanged();
}

void KisToolBrushNew::slotSetTailAggressiveness(qreal argh_rhhrr)
{
...
void KisToolBrushNew::setSmoothPressure(bool value)
{
    smoothingOptions()->setSmoothPressure(value);
}

void KisToolBrushNew::slotSetMagnetism(int magnetism)
{
    m_magnetism = expf(magnetism / (double)MAXIMUM_MAGNETISM) / expf(1.0);
}

bool KisToolBrushNew::useScalableDistance() const
{
    return smoothingOptions()->useScalableDistance();
}

// used with weighted smoothing
void KisToolBrushNew::setUseScalableDistance(bool value)
{
...
void KisToolBrushNew::resetCursorStyle()
{
    KisConfig cfg(true);
...
bool KisToolBrushNew::useDelayDistance() const
{
    return smoothingOptions()->useDelayDistance();
}

qreal KisToolBrushNew::delayDistance() const
{
    return smoothingOptions()->delayDistance();
}

void KisToolBrushNew::setUseDelayDistance(bool value)
{
...
void KisToolBrushNew::setDelayDistance(qreal value)
{
    smoothingOptions()->setDelayDistance(value);
    Q_EMIT delayDistanceChanged();
}

void KisToolBrushNew::setFinishStabilizedCurve(bool value)
{
...
bool KisToolBrushNew::finishStabilizedCurve() const
{
    return smoothingOptions()->finishStabilizedCurve();
}

void KisToolBrushNew::setStabilizeSensors(bool value)
{
    smoothingOptions()->setStabilizeSensors(value);
    Q_EMIT stabilizeSensorsChanged();
}

bool KisToolBrushNew::stabilizeSensors() const
{
    return smoothingOptions()->stabilizeSensors();
}

void KisToolBrushNew::updateSettingsViews()
{
...
QWidget * KisToolBrushNew::createOptionWidget()
{
...
QList<QAction *> KisToolBrushNewFactory::createActionsImpl()
{
